...
精度确实并不是语言的问题，而是浮点数存储本身固有的缺陷。浮点数无法很精确表示其树枝范围内的所有值，只能精确表示可用科学计数法 `m*2^e` 表示的数值而已，比如 0.5 的科学计数法是 `2^-1`，则可被精确存储了；而 0.1，0.2 则无法被精确存储。
...
引入`Number.EPSILON`(2.220446049250313e-16)这么小的量，在于为浮点数设置一个误差范围，如果误差小雨`EPSILON`，则认为结果是可靠的。
